...
namespace Game.Utils
{
    ...
    public static class DebugExtensions
    {
        ...
        public static void DebugCircle(Vector3 position, Vector3 up, Color color, float radius = 1.0f,
            float duration = 0, bool depthTest = true)
        {
            Vector3 new_up = up.normalized * radius;
            Vector3 new_forward = Vector3.Slerp(new_up, -new_up, 0.5f);
            Vector3 new_right = Vector3.Cross(new_up, new_forward).normalized * radius;

            var matrix = new Matrix4x4();

            matrix[0] = new_right.x;
            matrix[1] = new_right.y;
            matrix[2] = new_right.z;

            matrix[4] = new_up.x;
            matrix[5] = new_up.y;
            matrix[6] = new_up.z;

            matrix[8] = new_forward.x;
            matrix[9] = new_forward.y;
            matrix[10] = new_forward.z;
        ...
        public static void DrawArrowSimple(Vector3 position, Vector3 direction, Color color)
        {
            var oldColor = Gizmos.color;
            Gizmos.color = color;
        ...
        public static void DrawArrowSimple(Vector3 position, Vector3 direction)
        {
            DrawArrowSimple(position, direction, Color.white);
        }

        private static void DrawArrowEnd(Vector3 position, Vector3 direction, Color color, float angle = 20.0f,
            float arrowHeadLength = 0.15f, float arrowPosition = 1f)
        {
            float length = direction.magnitude;

            var up = Vector3.Slerp(direction, -direction, 0.5f);
            var right = Vector3.Cross(direction, up).normalized * length;

            var slerpedVector = Vector3.Slerp(direction, up, angle / 90.0f);


            var oldColor = Gizmos.color;
            Gizmos.color = color;

            Gizmos.DrawRay(position, slerpedVector.normalized * arrowHeadLength);
            Gizmos.DrawRay(position, Vector3.Slerp(direction, -up, angle / 90.0f).normalized * arrowHeadLength);
            Gizmos.DrawRay(position, Vector3.Slerp(direction, right, angle / 90.0f).normalized * arrowHeadLength);
            Gizmos.DrawRay(position, Vector3.Slerp(direction, -right, angle / 90.0f).normalized * arrowHeadLength);
